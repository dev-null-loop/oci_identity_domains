variable "action" {
  description = "(Optional) Requestor can set action to CANCEL to cancel the request or to ESCALATE to escalate the request while the request status is IN_PROGRESS. Requestor can't escalate the request if canceling or escalation is in progress."
  type        = string
  default     = null
}
...
variable "requesting" {
  description = "(Required) Requestable resource reference."
  type = object({
    type  = string
    value = string
  })
...
variable "requestor" {
  description = "(Optional) Requesting User"
  type = object({
    value = string
  })
  default = null
