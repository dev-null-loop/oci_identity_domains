variable "active" {
  description = "(Optional) (Updatable) If true, rule is active"
  type        = bool
  default     = null
}
...
variable "condition" {
  description = "(Required) (Updatable) SCIM filters"
  type        = string
}

variable "condition_group" {
  description = "(Optional) (Updatable) Condition or ConditionGroup assigned to the rule"
  type = object({
    type  = string
...
variable "locked" {
  description = "(Optional) (Updatable) Shows if the rule is locked"
  type        = bool
  default     = null
}

variable "name" {
  description = "(Required) (Updatable) Rule name"
  type        = string
}
...
variable "policy_type" {
  description = "(Required) (Updatable) PolicyType on which the policy is based"
  type = object({
    value = string
  })
}
...
variable "return" {
  description = "(Required) (Updatable) The return values are the then portion of a Rule"
  type = list(object({
    name          = string
    return_groovy = optional(string)
    value         = string
  }))
}

variable "rule_groovy" {
  description = "(Optional) (Updatable) The Groovy script that is run instead of the rule, if policy type allows the rule to be a Groovy script."
  type        = string
  default     = null
}
