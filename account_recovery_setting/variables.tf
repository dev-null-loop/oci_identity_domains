...
variable "factors" {
  description = "(Required) (Updatable) The account recovery factor used (for example, email, mobile number (SMS), security questions, mobile application push or TOTP) to verify the identity of the user and reset the user's password."
  type        = list(string)
}
...
variable "lockout_duration" {
  description = "(Required) (Updatable) Indicates how many minutes to disable account recovery for the user. The default value is 30 metric minutes."
  type        = number
}

variable "max_incorrect_attempts" {
  description = "(Required) (Updatable) Indicates the maximum number of failed account recovery attempts allowed for the user."
  type        = number
}
