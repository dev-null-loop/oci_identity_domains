...
variable "expires_on" {
  description = "(Optional) When the user's credentials expire."
  type        = string
  default     = null
}
...
variable "is_reset_secret" {
  description = "(Optional) Specifies whether the secret must be reset."
  type        = bool
  default     = null
}
...
variable "urnietfparamsscimschemasoracleidcsextensionself_change_user" {
  description = "(Optional) Controls whether a user can update themselves or not via User related APIs"
  type = object({
...
variable "user" {
  description = "(Optional) The user linked to the OAuth2 client credential."
  type = object({
